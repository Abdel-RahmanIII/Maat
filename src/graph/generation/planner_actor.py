"""Planner-Actor subgraph — Strategist → Tactician → parse + validate."""

from __future__ import annotations

from typing import Any

from langgraph.graph import END, StateGraph
from langgraph.graph.state import CompiledStateGraph

from src.agents.strategist import create_plan
from src.agents.tactician import execute_plan
from src.config import ModelConfig
from src.context import ConversationContext
from src.graph.base_graph import parse_and_validate
from src.state import TurnState


def build_planner_actor_subgraph(
    model_config: ModelConfig | None = None,
    context: ConversationContext | None = None,
) -> CompiledStateGraph:
    """Build the Planner-Actor three-node generation subgraph.

    Nodes: strategist → tactician → parse_validate

    If *context* is provided, agent nodes will read/write conversation
    history for their respective agent IDs.
    """

    cfg = model_config
    ctx = context

    def _strategist(state: TurnState) -> dict[str, Any]:
        """Strategist LLM produces a natural-language plan."""

        history = ctx.get_history("strategist") if ctx else None

        result = create_plan(
            fen=state["board_fen"],
            move_history=state["move_history"],
            input_mode=state.get("input_mode", "fen"),
            model_config=cfg,
            conversation_history=history,
        )
        return {
            "strategic_plan": result["plan"],
            "llm_calls_this_turn": state["llm_calls_this_turn"] + 1,
            "tokens_this_turn": (
                state["tokens_this_turn"]
                + result["prompt_tokens"]
                + result["completion_tokens"]
            ),
            "prompt_token_count": state["prompt_token_count"] + result["prompt_tokens"],
            "threat_report": "",  # Clear TA field
        }

    def _tactician(state: TurnState) -> dict[str, Any]:
        """Tactician LLM converts the strategic plan into a UCI move."""

        history = ctx.get_history("tactician") if ctx else None

        result = execute_plan(
            fen=state["board_fen"],
            move_history=state["move_history"],
            strategic_plan=state["strategic_plan"],
            feedback_history=state["feedback_history"],
            input_mode=state.get("input_mode", "fen"),
            model_config=cfg,
            conversation_history=history,
        )
        return {
            "raw_llm_response": result["raw_output"],
            "llm_calls_this_turn": state["llm_calls_this_turn"] + 1,
            "tokens_this_turn": (
                state["tokens_this_turn"]
                + result["prompt_tokens"]
                + result["completion_tokens"]
            ),
            "prompt_token_count": state["prompt_token_count"] + result["prompt_tokens"],
            "_gen_raw_output": result["raw_output"],
        }

    def _parse_validate(state: TurnState) -> dict[str, Any]:
        """Parse the raw LLM output and validate it against python-chess."""

        pv = parse_and_validate(state["_gen_raw_output"], state["board_fen"])
        attempts = state["total_attempts"] + 1

        error_types = list(state["error_types"])
        if not pv["is_valid"] and pv["error_type"]:
            error_types.append(pv["error_type"])

        return {
            "proposed_move": pv["proposed_move"],
            "is_valid": pv["is_valid"],
            "first_try_valid": (
                pv["is_valid"] if attempts == 1 else state["first_try_valid"]
            ),
            "total_attempts": attempts,
            "error_types": error_types,
        }

    # ── Phase 3: conditional entry point for Agent 1 caching ──
    def _should_skip_strategist(state: TurnState) -> str:
        """Skip Strategist if plan is already cached (retry scenario)."""
        if state.get("strategic_plan"):
            return "tactician"
        return "strategist"

    graph = StateGraph(TurnState)
    graph.add_node("strategist", _strategist)
    graph.add_node("tactician", _tactician)
    graph.add_node("parse_validate", _parse_validate)

    graph.set_conditional_entry_point(
        _should_skip_strategist,
        {"strategist": "strategist", "tactician": "tactician"},
    )
    graph.add_edge("strategist", "tactician")
    graph.add_edge("tactician", "parse_validate")
    graph.add_edge("parse_validate", END)

    return graph.compile()
